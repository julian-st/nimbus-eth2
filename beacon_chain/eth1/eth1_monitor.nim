...
proc createInitialDepositSnapshot*(
    ...
    engineApiUrl: EngineApiUrl): Future[Result[DepositContractSnapshot, string]]
    {.async.} =

  let
    # TODO: do we need timeout here?
    rpcClient = await ELConnection.new(engineApiUrl).connectedRpcClient()
    ...
    if depositContractDeployedAt.isHash:
      ...
      try:
        var blk = awaitWithRetries(
          rpcClient.getBlockByNumber(depositContractDeployedAt.number),
          web3RequestsTimeout)
...
proc startExchangeTransitionConfigurationLoop(m: ELManager) {.async.} =
  if m.shouldExchangeTransitionConfiguration and not m.hasProperlyConfiguredConnection:
    ...
    if not m.hasProperlyConfiguredConnection and m.requireEngineAPI:
...
proc startDepositsSyncingLoop(m: ELManager) {.async.} =
  ...
  let
    shouldProcessDeposits = not m.depositContractAddress.isZeroMemory
        ...
        if m.currentEpoch >= m.cfg.BELLATRIX_FORK_EPOCH and m.terminalBlockHash.isNone:
          ...
          if terminalBlockCandidate.totalDifficulty >= m.cfg.TERMINAL_TOTAL_DIFFICULTY:
            while not terminalBlockCandidate.parentHash.isZeroMemory:
              var parentBlock = try:
                awaitWithRetries(
                  rpcClient.getBlockByHash(terminalBlockCandidate.parentHash),
                  web3RequestsTimeout)
              except CatchableError as err:
                error "Failed to obtain block details for TTD detection", err = err.msg
                raise err
